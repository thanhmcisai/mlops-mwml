...
from mlops import data, predict, utils, evaluate
...
def train(args: Namespace, df: pd.DataFrame, trial: optuna.trial.Trial = None) -> Dict[str, Any]: # type: ignore
    ...
    # Preproces
    df = data.preprocess(df, lower=args.lower,  # type: ignore
                         stem=args.stem, min_freq=args.min_freq)  # type: ignore
    label_encoder = data.LabelEncoder().fit(df.tag)  # type: ignore

    # Split data
    X_train, X_val, X_test, y_train, y_val, y_test = \
        data.get_data_splits(X=df.text.to_numpy(),  # type: ignore
                             y=label_encoder.encode(df.tag))  # type: ignore
    test_df = pd.DataFrame(
        {"text": X_test, "tag": label_encoder.decode(y_test)})  # type: ignore
    ...
    for epoch in range(args.num_epochs):
        ...
        train_loss = log_loss(
            y_train, model.predict_proba(X_train))  # type: ignore
    ...
    args.threshold = np.quantile(  # type: ignore
        [y_prob[i][j] for i, j in enumerate(y_pred)], q=0.25)  # Q1 # type: ignore
    ...
    y_pred = predict.custom_predict(  # type: ignore
        y_prob=y_prob, threshold=args.threshold, index=other_index)  # type: ignore

...
def test_replace_minority_labels(df: pd.DataFrame, min_freq: int, unique_labels: List[str]):
...
def test_clean_text(text: str, lower: bool, stem: bool, stopwords: List[str], cleaned_text: str):
    assert (
        data.clean_text(
            text=text,
            lower=lower,
...
class TestLabelEncoder:
    ...
    def test_save_and_load(self):
        with tempfile.TemporaryDirectory() as dp:
            ...
            self.label_encoder.save(file_path=str(fp))
            label_encoder = data.LabelEncoder.load(file_path=str(fp))
            assert len(label_encoder.classes) == 0
